        ...
        <div class="filter-inset mCustomScrollbar">
            <form>
                ...
                <div class="form-group">
                    ...
                    <div ng-repeat="role in roles" class="checkbox"><label><input ng-model="selected_roles[$index]" ng-change="toggle_check_roles($index, role.name)" type="checkbox" ng-true-value="'{{role.name}}'" ng-false-value=""> {{role.label}}</label></div>
                </div>
                <div class="form-group">
                    <div class="category-block">
                        <h5><?php echo $this->lang->line('departments'); ?></h5>
                        <div ng-repeat="tag in tags" class="checkbox"><label><input ng-model="selected_tags[$index]" ng-change="toggle_check_tags($index, tag.id)" type="checkbox" ng-true-value="'{{tag.id}}'" ng-false-value=""> {{tag.tag_name}}</label></div>
                    </div>
                </div>
            </form>
        </div>
