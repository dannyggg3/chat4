...
class CP_Model extends CI_Model {
    ...
    public function __construct() {
        ...
        $ut = TABLE_USERS;
        $this->user_select_fields = $ut . '.id, ' . $ut . '.name, ' . $ut . '.email, ' . $ut . '.display_name, ' . $ut . '.profile_pic, ' . $ut . '.role, ' . $ut . '.last_activity_time, ' . $ut . '.last_login, ' . $ut . '.contact_number';
    ...
    function get_agents($department_id = '', $select = '') {
        if (empty($select)) {
        ...
        // if department id 
        if ($department_id) {
            $this->db->where('tag_id', $department_id);
        }
        ...
        // if department id 
        if ($department_id) {
            $query->join(TABLE_USER_TAGS, TABLE_USER_TAGS . '.user_id = ' . TABLE_USERS . '.id', 'left');
        }
    ...
    function get_online_agents($department_id = '') {
        $result = array();
        $online_activity_time = date("Y-m-d H:i:s", (now() - (15)));
        $mobile_online_activity_time = date("Y-m-d H:i:s", (now() - (5 * 60)));
        $desktop_online_activity_time = date("Y-m-d H:i:s", (now() - (5 * 60)));
    ...
    function get_online_agents_list($department_id = '') {
        $online_users = array();
        $online_activity_time = date("Y-m-d H:i:s", (now() - (15)));
        $mobile_online_activity_time = date("Y-m-d H:i:s", (now() - (5 * 60)));
        $desktop_online_activity_time = date("Y-m-d H:i:s", (now() - (5 * 60)));
    ...
    function get_notifications($agetn_id) {
        ...
        $notifications = $this->db->select(TABLE_NOTIFICATIONS . '.id, '
                        ...
                        . 'sender.name, '
                        . 'sender.email, '
                        . " CASE WHEN sender.profile_pic = '' THEN CONCAT(CONCAT(CONCAT('http://www.gravatar.com/avatar/', MD5(LOWER(sender.email))),'?d=404'), sender.profile_pic) ELSE CONCAT('" . base_url(UPLOAD_DIR . '/' . PROFILE_PICS) . "/thumb/', sender.profile_pic) END  as profilePic"
    ...
    function closeClosedSessions() {
        ...
        $sessions = $this->db->select(TABLE_CHAT_SESSIONS . '.id, ( select sqm.created_at from ' . TABLE_CHAT_MESSAGES . ' sqm where sqm.sort_order = max(' . TABLE_CHAT_MESSAGES . '.sort_order) ) as created_at')
    ...
    function increase_visitors_counter() {
        $created_at = date("Y-m-d", now());
        // fetch message by local id
        $query = $this->db->get_where(TABLE_DAILY_VISITORS, array('created_at' => $created_at));
        $record = $query->row();
        if ($record) {
            ...
            $inserData = array('visitors' => 1, 'created_at' => $created_at);
    ...
    function increase_pagevies_counter($pagevieData) {
        $created_at = date("Y-m-d", now());
        // fetch message by local id
        $query = $this->db->get_where(TABLE_DAILY_PAGEVIEWS, array('created_at' => $created_at, 'page_url' => $pagevieData['page_url']));
        $record = $query->row();
        if ($record) {
