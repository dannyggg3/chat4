...
class Orequests extends CP_AgentController {
    ...
    function get_requests() {
        ...
        if ($this->current_user) {
            $offline_requests = $this->orequest->get_offline_requests($this->current_user->id, $this->input->post('offset'), true);
        }

        return $this->output->set_content_type('application/json')->set_output($this->return_json($offline_requests));
    }
