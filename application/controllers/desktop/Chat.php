...
class Chat extends CP_AppController {
     ...
     * @param $user_id
     * 
     * @param Post params = agent_ids, chat_session_id
     * 
    ...
    function forward() {
        ...
        if ($this->valid_token) {
            ...
            if ($this->form_validation->run() === true) {
                $this->chat_request->model_data = $this->input->post(array('agent_ids', 'chat_session_id'));
                $this->chat_request->model_data['request_type'] = 'forward';
                $this->chat_request->model_data['message'] = 'You have a new forwarded chat request.';

                $visitor = $this->chat_user->get_visitor($this->input->post('chat_session_id'));
     ...
     * @param $user_id
     * 
     * @param Post params = department_ids, chat_session_id
     * 
     * @return $error or $message
     */
    ...
    function forward_to_departments() {
        ...
        if ($this->valid_token) {
            ...
            if ($this->form_validation->run() === true) {
                $this->chat_request->model_data = $this->input->post(array('department_id', 'chat_session_id'));
                $this->chat_request->model_data['request_type'] = 'forward';
                $this->chat_request->model_data['message'] = 'You have a new forwarded chat request.';
     ...
     *  
     * @param Get params = chat_session_id
     * 
     * @return $error or $message
     */
    ...
    function end() {
        ...
        if ($this->valid_token) {
            if ($chat_session_id) {
                ...
                $response['error'] = 'Chat Session ID is required.';
            }
        } else {
            $response['error'] = $this->lang->line('invalid_token');
        }
    ...
    public function agents_list() {
        ...
        if ($this->valid_token) {
            if ($chat_session_id) {
                $agents_list = $this->chat_session->get_available_agents($chat_session_id);
    ...
    public function departments_list() {
        $response = array('result' => 'failed');
        if ($this->valid_token) {
            $departments_list = $this->chat_session->get_available_departments();
        ...
        if ($this->valid_token) {
            ...
            if ($this->form_validation->run() === true) {
                ...
                if ($message_id and is_object($message_id) === false) {
                    ...
                    $message['displayMessage'] = 'You have new message from ' . $sender->name;
                    $message['chatSessionId'] = $this->input->post('chat_session_id');
    ...
    public function upload_file() {
        ...
        if ($this->valid_token) {
            ...
            if ($this->form_validation->run() === true) {
                ...
                $allowed_types = str_replace(".", "", str_replace(" ", "", $this->settings->allowed_filetypes));
                ...
                if (isset($filedata['error'])) {
                    ...
                    $response['errors'] = $filedata['error'] .  $extra_data;
                } else {
        ...
        if ($this->valid_token) {
            ...
            if ($chat_session->session_status == 'requested' or $chat_session->session_status == 'forward') {
                if ($request->request_status == 'pending') {
                    ...
                    $response['settings'] = $this->configuration->get_agent_settings();
                    $response['result'] = 'success';
                } else {
    ...
    function reply_offline_request() {
        ...
        if ($this->valid_token) {
            ...
            if ($this->form_validation->run() === true) {
                ...
                if ($settings->can_reply_attended_orequests != 'yes') {
                    if ($request->request_status == 'pending') {
