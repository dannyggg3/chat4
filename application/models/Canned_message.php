...
class Canned_message extends CP_Model {
    ...
    function filter_message ($filters = array()) {
        ...
        $select = 'cm.*, '
                ...
                . 'users.profile_pic, '
                . " CASE WHEN users.profile_pic = '' THEN CONCAT(CONCAT(CONCAT('http://www.gravatar.com/avatar/', MD5(LOWER(users.email))),'?s=200'), users.profile_pic) ELSE CONCAT('" . base_url(UPLOAD_DIR . '/' . PROFILE_PICS) . "/thumb/', users.profile_pic) END  as userProfilePic";
        
        $query = $this->db->select($select);
        
        if (isset($filters['keywords']) and $filters['keywords']) {
            $query->like('cm.title', $filters['keywords']);
            $query->like('cm.description', $filters['keywords']);
        }
