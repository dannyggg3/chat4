(function () {
    angular.module('canned-messages', []).directive('cannedMessage', function ($timeout, $uibModal, $document) {
        var templateUrl;
        try {
        ...
        $scope.getMessages = function () {
            if ($scope.options.listUrl) {
                $http.get($scope.options.listUrl).then(function (response) {
                    $scope.canned_messages = response.data;
                }, function (response) {
        ...
        $scope.cancelCannedModal = function () {
            ...
            var $buttonHtml = '<i role="button" class="fa fa-envelope" ng-click="openCannedModal()" title="{{options.selector_title}}"></i>';
            ...
            var $mHeader = '<div class="modal-header">'
                    ...
                    + '<h3 class="modal-title" id="modal-title">{{showForm? form_title : options.title}}</h3>'
                    + '</div>';
            ...
            var $templateHtml = $mHeader
                    ...
                    + '</div>'
                    + '</div>'
                    + '<a ng-hide="showForm" class="btn btn-circle btn-primary btn-fixed" href="#" ng-click="addMessage($event)" title="{{options.canned_add_new}}">+</a>';
