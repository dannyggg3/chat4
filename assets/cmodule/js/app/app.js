var app;
(function () {
    app = angular.module("cmodule", ['cmodule.filters', 'cmodule.services', 'ngSanitize', 'ngAnimate', 'colorpicker.module', 'checklist-model', 'canned-messages', 'ui.bootstrap', 'angular-smilies', 'naif.base64'], function ($httpProvider) {
    ...
    app.controller("BodyController", function ($http, $scope, alertService, $timeout, Setting) {
        ...
        $scope.accept_request = function (event, row) {
            ...
            $http.post(site_url + "?d=agents&c=chat&m=accept_request&request_id=" + row.id + '&chat_session_id=' + row.chat_session_id).success(function (response) {
                if (response.result == 'success') {
