(function () {
    app.controller("ProfileController", ['$scope', '$http', '$interval', '$window',
        function ($scope, $http, $interval, $window) {
            ...
            $scope.save_user = function (event) {
                ...
                $http.post(site_url + "?d=agents&c=agents&m=update_profile&id=" + $scope.user.id, formdata, {
            ...
            $scope.save_crop_image = function ($image, uid) {
                angular.element(document).off('click.confirm').on("click.confirm", "#crop-image", function () {
                    ...
                    $http.post(site_url + "?d=agents&c=agents&m=crop_user_picture&id=" + uid, $scope.cropData).success(function (response) {
                        if (response.result == 'success') {
                            var imageUrl = response.profile_picture + '?decache=' + Math.random();
                            $scope.user.profile_picture = imageUrl;
                            $scope.user.profile_pic = response.profile_pic;
